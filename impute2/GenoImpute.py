...
def phase(input, sample, outdir, chrom):
    name = os.path.basename(input).split(".")[0]
    map = "/share/data0/reference/Genome/ALL_1000G_phase3integrated_v3_impute/" + \
        "genetic_map_" + chrom + "_combined_b37.txt"
    cmd = "source /etc/profile.d/set.sh\n"
    cmd = cmd + "export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/share/apps/glibc.2.14/lib\n"
    cmd = cmd + "shapeit "
    cmd = cmd + "--input-gen " + input + " " + sample + " "
    cmd = cmd + "--input-map " + map + " "
    cmd = cmd + "--output-max " + outdir + "/" + \
        name + "_" + chrom + "_phased.haps "
    cmd = cmd + "--thread 8"
    return(cmd)


def impute(input, outdir, chrom, start, end):
    name = os.path.basename(input).split(".")[0]
    name = name + "_" + start + "-" + end + "_impute.gen"
    cmd = "source /etc/profile.d/set.sh\n"
    cmd = cmd + "impute2 "
    cmd = cmd + "-known_haps_g " + input
    cmd = cmd + " -h /share/data0/reference/Genome/ALL_1000G_phase3integrated_v3_impute/1000GP_Phase3_" + chrom + ".hap.gz "
    ...
    cmd = cmd + "-int " + start + " " + end + " -Ne 20000 "
    cmd = cmd + "-o " + outdir + "/" + name
    return(cmd)
...
if __name__ == '__main__':
    ...
    if args.subcom == "phase":
        cmd = phase(args.geno, args.sample, args.outdir, args.chrom)
    elif args.subcom == "impute":
        cmd = impute(args.geno, args.outdir, args.chrom, args.start, args.end)
    if args.qsub:
        qsub(args, cmd)
    else:
        print(cmd)
